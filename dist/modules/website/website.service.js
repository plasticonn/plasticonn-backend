...
exports.Website = exports.websiteData = void 0;
...
const websiteData = async () => {
    ...
    // ── Parallel queries ──────────────────────────────────────────────
    const [activeCollectors, totalCollectionCenters, totalRecyclingCenters, allDrops, monthlyTrend,] = await Promise.all([
        // Hero
        collectors_model_1.CollectorsModel.countDocuments({ status: "active" }),
        centers_model_1.CenterModel.countDocuments({ centerType: "collection", status: "active" }),
        centers_model_1.CenterModel.countDocuments({ centerType: "recycling", status: "active" }),
        // All drops (used across multiple sections)
        drops_model_1.DropsModel.find({}, { amount: 1, status: 1, collector_id: 1, center_id: 1, createdAt: 1 }).lean(),
    ...
    // ── Derived stats ─────────────────────────────────────────────────
    const totalPlasticsCollected = allDrops.reduce((sum, d) => sum + (d.amount ?? 0), 0);
    const recycledDrops = allDrops.filter((d) => d.status === "recycled");
    const totalRecycled = recycledDrops.reduce((sum, d) => sum + (d.amount ?? 0), 0);
    const percentRecycled = totalPlasticsCollected > 0
        ? Math.round((totalRecycled / totalPlasticsCollected) * 100)
        : 0;
    // Unique active participants (collectors + centers referenced in drops)
    const uniqueCollectorIds = new Set(allDrops.map((d) => d.collector_id?.toString()).filter(Boolean));
    const uniqueCenterIds = new Set(allDrops.map((d) => d.center_id?.toString()).filter(Boolean));
    ...
    const monthsWithData = new Set(allDrops.map((d) => {
        const date = new Date(d.createdAt);
        return `${date.getFullYear()}-${date.getMonth()}`;
    }));
    const avgPerMonth = monthsWithData.size > 0
...
exports.Website = {
    websiteData: exports.websiteData,
};
